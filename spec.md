...
&#x20;  - Format: `claude/feature/<feature\_slug>`

&#x20;  - Example: `claude/feature/card-component`.



If you cannot infer a sensible `feature\_title` and `feature\_slug`, ask the user to clarify instead of guessing.
...
Before making any content, switch to a new Git branch using the `branch\_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `claude/feature/card-component-01`
...
Create a markdown spec document that Plan mode can use directly and save it in the \_specs folder using the `feature\_slug`. Use the exact structure as defined in the spec template file here: @\_specs/template.md. Do not add technical implementation details such as code examples.
...
Spec file: specs/<feature\_slug>.md
